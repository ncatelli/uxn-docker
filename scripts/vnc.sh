...
mkdir -p /var/run/dbus /usr/local/etc/usersession/
...
startInBackgroundIfNotRunning "Xtigervnc" sudoUserIf "tigervncserver \${DISPLAY} -geometry \${screen_geometry} -depth \${screen_depth} -rfbport ${VNC_PORT} -dpi \${VNC_DPI:-96} -localhost -desktop fluxbox -fg -passwd /usr/local/etc/usersession/vnc-passwd"
...
echo "${VNC_PASSWORD}" | vncpasswd -f > /usr/local/etc/usersession/vnc-passwd
